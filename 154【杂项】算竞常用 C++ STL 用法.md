...
for (set<int>::iterator it = st.begin(); it != st.end(); ++it)
    cout << *it << endl;
//其中set<int>::iterator是在声明一个迭代器；it是自己命名的变量名，初值是st.begin()(集合的头迭代器)，停止条件是当头迭代器和尾迭代器相等（即把整个集合遍历完了），++it就是迭代器递增；输出是就是解引用it
```
...
```cpp
//输出代码
for (auto &ele : st)//auto表示的是自动识别数据类型，如果知道是int的话改成int也可以
    cout << ele << endl;
```
